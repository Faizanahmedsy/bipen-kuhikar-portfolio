...
import type { SiteContent } from "@/lib/sanity"

if (typeof window !== "undefined") {
  gsap.registerPlugin(ScrollTrigger)
}

// Map color keys → Tailwind classes
...
// Icon map for benefits by index
const benefitIcons = [
  GraduationCap,
  ShieldCheck,
  TrendingUp,
...
export default function HomePage({ data }: { data: SiteContent }) {
    ...
    >
      ...
      <nav className="sticky top-0 z-50 border-b border-slate-100 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl animate-in items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Phone size={14} /> {data.phoneDisplay}
            </a>
            <a
              href={`tel:${data.phoneNumber}`}
          ...
          <div className="group relative overflow-hidden rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-2xl transition-transform duration-500 hover:scale-[1.01]">
            ...
            <div className="relative z-10">
              ...
              <div className="space-y-3 pt-2">
                {data.heroCardChecklist.map((text, i) => (
                  <div
                    key={i}
          ...
          <div className="reveal mb-16 text-center">
            <h2 className="mb-4 text-4xl leading-none font-black tracking-tight text-slate-900">
              ...
              <span className="text-emerald-600 italic">
                {data.benefitsSectionTitleAccent}
              </span>
            </h2>
            <p className="mx-auto max-w-2xl text-lg font-medium text-slate-500">
              {data.benefitsSectionDescription}
            </p>
          </div>

          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {data.benefits.map((benefit, i) => {
              const colors = colorMap[benefit.colorKey] ?? colorMap.indigo
              const Icon = benefitIcons[i % benefitIcons.length]
              return (
                <div
                  key={i}
                  className={`reveal rounded-3xl border bg-white p-6 shadow-sm ${colors.border} group relative cursor-pointer overflow-hidden transition-all duration-300 hover:shadow-xl`}
                >
                  <div
                    className={`absolute -top-4 -right-4 h-24 w-24 rounded-full ${colors.bg} opacity-50 transition-transform duration-500 group-hover:scale-150`}
                  />
                  <div className="relative z-10 mb-6 flex items-center gap-4">
                    <div
                      className={`h-14 w-14 rounded-2xl ${colors.bg} ${colors.color} flex items-center justify-center shadow-inner`}
                    >
                      <Icon size={32} />
                    </div>
                  ...
                  <div className="relative z-10 space-y-3">
                    <div className="flex gap-1">
                      <div className="relative h-12 flex-1 overflow-hidden rounded-xl border border-slate-100 bg-slate-50">
                        <div className="absolute bottom-0 left-2 h-[30%] w-2 rounded-t-sm bg-slate-200 transition-all duration-700 group-hover:h-[60%]" />
                        <div className="absolute bottom-0 left-5 h-[80%] w-2 rounded-t-sm bg-slate-200 transition-all duration-700 group-hover:h-[40%]" />
                        <div className="absolute bottom-0 left-8 h-[50%] w-2 rounded-t-sm bg-slate-200 transition-all duration-700 group-hover:h-[90%]" />
                      </div>
                      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-slate-100 bg-slate-50">
                        <ArrowRight
                          size={16}
                          className={`text-slate-300 transition-colors ${colors.accent}`}
                        />
                      </div>
                    </div>
                  </div>
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-16 px-6 py-24 md:grid-cols-2">
        <div className="reveal space-y-8">
          ...
          <h2 className="text-4xl leading-[0.9] font-black tracking-tighter text-slate-900 md:text-5xl">
            {data.irdaSectionTitle} <br />
            <span className="text-emerald-600 italic">
              {data.irdaSectionTitleAccent}
            </span>
          </h2>
          <p className="text-lg leading-relaxed font-medium text-slate-500">
            {data.irdaSectionDescription}
          </p>
        </div>
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <div className="flex w-full items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div className="font-mono text-[10px] font-black tracking-widest text-slate-300">
                  REG: {data.phoneNumber.replace("+91", "")}
                </div>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <div className="reveal order-1 space-y-8 md:order-2">
          ...
          <h2 className="text-4xl leading-[0.9] font-black tracking-tighter text-slate-900 md:text-5xl">
            {data.aboutSectionTitle} <br />
            <span className="text-emerald-600 italic">
              {data.aboutSectionTitleAccent}
            </span>
          </h2>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="font-bold text-slate-900">
                  {data.aboutFeatureTitle}
                </p>
                <p className="text-sm font-medium text-slate-400">
                  {data.aboutFeatureDescription}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24">
        <div className="group relative space-y-8 overflow-hidden rounded-[3rem] border border-slate-200 bg-white p-12 text-center shadow-2xl transition-all hover:shadow-emerald-100 md:p-20">
          ...
          <div className="relative z-10 flex flex-col justify-center gap-4 pt-4 sm:flex-row">
            ...
            >
              <Phone size={20} /> {data.ctaCallText}
            </a>
            <a
            ...
            >
              <Mail size={20} /> {data.ctaEmailText}
            </a>
          </div>
        </div>
      </section>
        ...
        <div className="relative mx-auto max-w-7xl px-6">
          ...
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            ...
            <p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">
              © {new Date().getFullYear()} {data.footerCopyrightName}
            </p>
          </div>
        </div>
      </footer>
